...
fn format_file(path: &Path, check: bool, indent_width: Option<usize>) -> Result<bool, String> {
    let (src, ast) = util::load_ast(path)?;
    let formatted = format::format_stmts(&ast, indent_width);
    if formatted == src {
        return Ok(false);
    }
    if check {
    ...
    fs::write(path, formatted).map_err(|e| format!("Không ghi được {}: {}", path.display(), e))?;
