...
#[no_mangle]
/// # Safety
/// Caller must provide valid RawValue pointers; out must be writable.
pub unsafe extern "C" fn jit_helper_len(arg: *const RawValue, out: *mut RawValue) -> i32 {
...
/// # Safety
/// Caller must provide valid RawValue pointers; out must be writable.
pub unsafe extern "C" fn jit_helper_index(
    target: *const RawValue,
    idx: *const RawValue,
    out: *mut RawValue,
) -> i32 {
...
/// # Safety
/// Caller must provide valid RawValue pointers; out must be writable.
pub unsafe extern "C" fn jit_helper_say(arg: *const RawValue, out: *mut RawValue) -> i32 {
    if let Some(value) = raw_to_value(arg) {
        println!("{}", value);
        write_raw(out, Value::Null);
        0
    } else {
        -1
    }
...
mod tests {
    ...
    fn len_helper_tests() {
        ...
        assert_eq!(unsafe { jit_helper_len(&raw, &mut out) }, 0);
        let res = Value::from_raw(&out);
    ...
    fn index_helper_tests() {
        ...
        let mut out = RawValue::null();
        assert_eq!(unsafe { jit_helper_index(&raw_list, &idx, &mut out) }, 0);
        let res = Value::from_raw(&out);
