...
fn dsu_new(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let n = args[0]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("dsu_new: n must be number", None))? as usize;
...
fn dsu_find(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let x = args[1]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("dsu_find: x must be number", None))? as usize;
    let mut parent = expect_list(
        dsu.get("p").unwrap_or(&Value::Null),
        "dsu_find: missing parent list",
    )?;
    let _rank = expect_list(
        dsu.get("r").unwrap_or(&Value::Null),
        "dsu_find: missing rank list",
    )?;
    let root = find_internal(x, &mut parent);
    dsu.insert("p".into(), Value::make_list(parent));
    Ok(Value::make_list(vec![
        Value::SmallInt(root as i64),
        Value::make_map(dsu),
    ]))
}
...
fn dsu_union(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let a = args[1]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("dsu_union: a must be number", None))?
        as usize;
    let b = args[2]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("dsu_union: b must be number", None))?
        as usize;
    let mut parent = expect_list(
        dsu.get("p").unwrap_or(&Value::Null),
        "dsu_union: missing parent list",
    )?;
    let mut rank = expect_list(
        dsu.get("r").unwrap_or(&Value::Null),
        "dsu_union: missing rank list",
    )?;
...
fn segtree_query(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let tree = expect_list(
        st.get("tree").unwrap_or(&Value::Null),
        "segtree_query: missing tree",
    )?;
    let size = st
        .get("size")
        .and_then(|v| v.as_i64())
        .ok_or_else(|| RuntimeError::new("segtree_query: missing size", None))?
        as usize;
    let mut l = args[1]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("l must be num", None))?
        + size as i64;
    let mut r = args[2]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("r must be num", None))?
        + size as i64;
...
fn segtree_update(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let mut tree = expect_list(
        st.get("tree").unwrap_or(&Value::Null),
        "segtree_update: missing tree",
    )?;
    let size = st
        .get("size")
        .and_then(|v| v.as_i64())
        .ok_or_else(|| RuntimeError::new("segtree_update: missing size", None))?
        as usize;
    let mut pos = args[1]
        .as_i64()
        .ok_or_else(|| RuntimeError::new("idx must be number", None))? as usize
        + size;
