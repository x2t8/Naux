...
pub const POLYVARIANT_R1_S3_VERSION: (u16, u16, u16) = (1, 2, 0);
pub const R1_S3_MAX_WORK_UNITS_HARD_CAP: u64 = 100_000_000;
pub const R1_S3_MAX_PARTIAL_VALUE_NODES_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_VARIANTS_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_CONTROL_SPLITS_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_DYNAMIC_PARAMETERS_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_HELPER_UNFOLDS_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_HELPER_DEPTH: usize = 256;
pub const R1_S3_MAX_RESIDUAL_NODES_HARD_CAP: u64 = 1_000_000;
pub const R1_S3_MAX_RESIDUAL_BYTES_HARD_CAP: u64 = 1_073_741_824;

const POLICY_DOMAIN: &[u8] = b"NAUX:core-n0:polyvariant-policy:r1-s3:v1\0";
const REQUEST_DOMAIN: &[u8] = b"NAUX:core-n0:polyvariant-request:r1-s3:v1\0";
const VERSION_KEY_DOMAIN: &[u8] = b"NAUX:core-n0:polyvariant-version-key:r1-s3:v1\0";

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PolyvariantR1S3Budget {
...
pub struct PolyvariantR1S3Usage {
    pub work_units: u64,
...
pub enum PolyvariantR1S3Pattern {
    KnownUnit,
    KnownBool(bool),
    KnownI64(i64),
    KnownF64(u64),
    KnownTuple(Vec<PolyvariantR1S3Pattern>),
    Hole {
        ty: Type,
        alias: u32,
    },
    Tuple(Vec<PolyvariantR1S3Pattern>),
    KnownSum {
        sum: SumType,
        constructor: u32,
        fields: Vec<PolyvariantR1S3Pattern>,
    },
    UnknownSum {
        sum: SumType,
        alias: u32,
...
pub struct PolyvariantR1S3Variant {
    source_function: FunctionId,
    residual_function: FunctionId,
    patterns: Vec<PolyvariantR1S3Pattern>,
}

impl PolyvariantR1S3Variant {
    ...
    pub fn patterns(&self) -> &[PolyvariantR1S3Pattern] {
        &self.patterns
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PolyvariantR1S3Report {
    ...
    usage: PolyvariantR1S3Usage,
    variants: Vec<PolyvariantR1S3Variant>,
    residual_hash: SemanticHash,
    residual_nodes: u64,
    residual_bytes: u64,
}

impl PolyvariantR1S3Report {
    ...
    pub fn budget(&self) -> PolyvariantR1S3Budget {
        self.budget
    }

    pub fn usage(&self) -> PolyvariantR1S3Usage {
        self.usage
    }

    pub fn variants(&self) -> &[PolyvariantR1S3Variant] {
        &self.variants
    }
...
pub struct PolyvariantR1S3Specialization {
    residual: ResidualCore,
    report: PolyvariantR1S3Report,
}

impl PolyvariantR1S3Specialization {
    ...
    pub fn report(&self) -> &PolyvariantR1S3Report {
        &self.report
    }
}

#[derive(Clone, Debug, PartialEq)]
pub enum PolyvariantR1S3Error {
    ZeroBudget {
        field: &'static str,
    },
...
impl fmt::Display for PolyvariantR1S3Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ZeroBudget { field } => write!(formatter, "R1-S3 budget {field} is zero"),
            Self::BudgetHardCapExceeded {
                ...
                formatter,
                "R1-S3 budget {field}={limit} exceeds hard cap {hard_cap}"
            ),
            Self::UnsupportedProfile(profile) => {
                write!(formatter, "R1-S3 admits P1V0, found {profile:?}")
            }
            Self::UnsupportedType {
                ...
                formatter,
                "R1-S3 function {} has unsupported {context} type {ty:?}",
                function.0
            ),
            Self::UnsupportedEffects { function } => write!(
                formatter,
                "R1-S3 function {} has effects outside empty or Error<Bounds>",
                function.0
            ),
            Self::UnsupportedRegionParameters { function } => write!(
                formatter,
                "R1-S3 function {} has region parameters outside [] or [RegionId(0)]",
                function.0
            ),
            Self::UnsupportedNode { function, node } => write!(
                formatter,
                "R1-S3 function {} contains unsupported {node}",
                function.0
            ),
            Self::UnsupportedPrimitive {
                ...
                formatter,
                "R1-S3 function {} contains unsupported primitive {primitive:?}",
                function.0
            ),
            Self::MissingFunction(function) => {
                write!(formatter, "R1-S3 cannot find function {}", function.0)
            }
            Self::MissingLocal { function, local } => write!(
                formatter,
                "R1-S3 function {} cannot resolve local {}",
                function.0, local.0
            ),
            Self::ArityMismatch {
                function,
                ...
                formatter,
                "R1-S3 call to function {} has {actual} argument(s), expected {expected}",
                function.0
            ),
            Self::InvalidEntrySlot { parameter } => write!(
                formatter,
                "R1-S3 entry parameter {} is not an admitted structural slot",
                parameter.0
            ),
            Self::MultipleRecursiveComponents { count } => write!(
                formatter,
                "R1-S3 reachable graph has {count} recursive components; at most two are admitted"
            ),
            Self::ExpectedBool { function } => write!(
                formatter,
                "R1-S3 function {} reached a non-Bool If condition",
                function.0
            ),
            Self::ExpectedTuple { function } => write!(
                formatter,
                "R1-S3 function {} reached a non-Tuple projection",
                function.0
            ),
            Self::ExpectedSum { function } => write!(
                formatter,
                "R1-S3 function {} reached a non-Sum Case",
                function.0
            ),
            Self::WorkBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_work_units {limit}")
            }
            Self::PartialValueBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_partial_value_nodes {limit}")
            }
            Self::VariantBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_variants {limit}")
            }
            Self::ControlBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_control_splits {limit}")
            }
            Self::DynamicParameterBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_dynamic_parameters {limit}")
            }
            Self::HelperBudgetExceeded { limit } => {
                write!(formatter, "R1-S3 exceeded max_helper_unfolds {limit}")
            }
            Self::FunctionIdExhausted => {
                formatter.write_str("R1-S3 exhausted the FunctionId namespace")
            }
            Self::LocalIdExhausted => {
                formatter.write_str("R1-S3 exhausted a residual LocalId namespace")
            }
            Self::AtomIdExhausted => formatter.write_str("R1-S3 exhausted internal AtomId"),
            Self::UnresolvedVariant => {
                formatter.write_str("R1-S3 finished with an unresolved variant")
            }
            Self::Residual(error) => write!(formatter, "R1-S3 residual failed: {error}"),
            Self::InternalInvariant { message } => {
                write!(formatter, "R1-S3 invariant failed: {message}")
            }
        }
    }
}

impl std::error::Error for PolyvariantR1S3Error {}
...
impl FreshLocals {
    ...
    fn allocate(&mut self) -> Result<LocalId, PolyvariantR1S3Error> {
        let local = self.next.ok_or(PolyvariantR1S3Error::LocalIdExhausted)?;
...
pub fn polyvariant_r1_s3_policy_hash() -> SemanticHash {
    let mut bytes = POLICY_DOMAIN.to_vec();
    put_version(&mut bytes, POLYVARIANT_R1_S3_VERSION);
    put_bytes(&mut bytes, VERSION_KEY_DOMAIN);
    for cap in [
        R1_S3_MAX_WORK_UNITS_HARD_CAP,
        R1_S3_MAX_PARTIAL_VALUE_NODES_HARD_CAP,
        R1_S3_MAX_VARIANTS_HARD_CAP,
        R1_S3_MAX_CONTROL_SPLITS_HARD_CAP,
        R1_S3_MAX_DYNAMIC_PARAMETERS_HARD_CAP,
        R1_S3_MAX_HELPER_UNFOLDS_HARD_CAP,
        R1_S3_MAX_HELPER_DEPTH as u64,
        R1_S3_MAX_RESIDUAL_NODES_HARD_CAP,
        R1_S3_MAX_RESIDUAL_BYTES_HARD_CAP,
    ] {
    ...
    for capability in [
        ...
        b"residual-cap-min-s3-upstream-v1".as_slice(),
...
pub fn specialize_polyvariant_r1_s3(
    ...
    if source.profile != CoreProfile::P1V0 {
        return Err(PolyvariantR1S3Error::UnsupportedProfile(source.profile));
    }
    ...
    let mut machine = Machine {
        ...
        budget,
        usage: PolyvariantR1S3Usage::default(),
        next_atom: Some(0),
    ...
    let graph_edges = graph.values().try_fold(0_u64, |total, edges| {
        total
            ...
            .ok_or(PolyvariantR1S3Error::WorkBudgetExceeded {
    ...
    if recursive_components.len() > 2 {
        return Err(PolyvariantR1S3Error::MultipleRecursiveComponents {
    ...
    let entry = machine
        ...
        .get(&source.entry)
        .cloned()
        .ok_or(PolyvariantR1S3Error::MissingFunction(source.entry))?;
    if entry.parameters.len() != validated.request().entry_slots.len() {
        return Err(PolyvariantR1S3Error::ArityMismatch {
    ...
    let policy_hash = polyvariant_r1_s3_policy_hash();
    ...
    if machine.reserved.values().any(|version| !version.built) {
        return Err(PolyvariantR1S3Error::UnresolvedVariant);
    }
    ...
    .map_err(PolyvariantR1S3Error::Residual)?;
    let report = PolyvariantR1S3Report {
        policy_version: POLYVARIANT_R1_S3_VERSION,
        policy_hash,
        request_hash,
    ...
    Ok(PolyvariantR1S3Specialization { residual, report })
}

impl Machine {
    fn consume_work(&mut self, units: u64) -> Result<(), PolyvariantR1S3Error> {
        let next = self.usage.work_units.checked_add(units).ok_or(
            PolyvariantR1S3Error::WorkBudgetExceeded {
                limit: self.budget.max_work_units,
            },
        )?;
        if next > self.budget.max_work_units {
            return Err(PolyvariantR1S3Error::WorkBudgetExceeded {
                limit: self.budget.max_work_units,
            });
        }
    ...
    fn consume_partial_nodes(&mut self, nodes: u64) -> Result<(), PolyvariantR1S3Error> {
        self.consume_work(nodes)?;
        let next = self.usage.partial_value_nodes.checked_add(nodes).ok_or(
            PolyvariantR1S3Error::PartialValueBudgetExceeded {
                limit: self.budget.max_partial_value_nodes,
            },
        )?;
        if next > self.budget.max_partial_value_nodes {
            return Err(PolyvariantR1S3Error::PartialValueBudgetExceeded {
                limit: self.budget.max_partial_value_nodes,
            });
        }
    ...
    fn consume_control(&mut self, splits: u64) -> Result<(), PolyvariantR1S3Error> {
        let next = self.usage.control_splits.checked_add(splits).ok_or(
            PolyvariantR1S3Error::ControlBudgetExceeded {
                limit: self.budget.max_control_splits,
            },
        )?;
        if next > self.budget.max_control_splits {
            return Err(PolyvariantR1S3Error::ControlBudgetExceeded {
                limit: self.budget.max_control_splits,
            });
        }
    ...
    fn consume_helper(&mut self) -> Result<(), PolyvariantR1S3Error> {
        if self.usage.helper_unfolds == self.budget.max_helper_unfolds {
            return Err(PolyvariantR1S3Error::HelperBudgetExceeded {
    ...
    fn allocate_atom(&mut self) -> Result<AtomId, PolyvariantR1S3Error> {
        let raw = self
            .next_atom
            .ok_or(PolyvariantR1S3Error::AtomIdExhausted)?;
    ...
    ) -> Result<AtomId, PolyvariantR1S3Error> {
    ...
    fn make_known(&mut self, value: SpecializationValue) -> Result<Partial, PolyvariantR1S3Error> {
        let nodes = specialization_value_nodes(&value)?;
        self.consume_partial_nodes(nodes)?;
    ...
    ) -> Result<Partial, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        match value {
            SpecializationValue::Unit => self.make_known(SpecializationValue::Unit),
            SpecializationValue::Bool(value) => self.make_known(SpecializationValue::Bool(*value)),
            SpecializationValue::I64(value) => self.make_known(SpecializationValue::I64(*value)),
            SpecializationValue::F64(value) => self.make_known(SpecializationValue::F64(
                f64::from_bits(canonical_f64_bits(*value)),
            )),
            SpecializationValue::Tuple(fields) => {
                let mut partials = Vec::with_capacity(fields.len());
                for field in fields {
                    partials.push(self.make_entry_static(field)?);
                }
            ...
            SpecializationValue::Sum {
                ...
                let expected = ty.constructors.get(*constructor as usize).ok_or_else(|| {
                    PolyvariantR1S3Error::InternalInvariant {
                        message: "validated entry Sum constructor disappeared".to_owned(),
                    }
                })?;
                if expected.fields.len() != fields.len() {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "validated entry Sum arity changed".to_owned(),
                    });
                }
                let mut partials = Vec::with_capacity(fields.len());
                for field in fields {
            ...
            SpecializationValue::ArrayF64(_) => Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 received an excluded static array".to_owned(),
            }),
        }
    }
    ...
    fn make_tuple(&mut self, fields: Vec<Partial>) -> Result<Partial, PolyvariantR1S3Error> {
        self.consume_work(fields.len() as u64)?;
        if let Some(values) = all_static(&fields) {
            return self.make_known(SpecializationValue::Tuple(values));
        }
        self.consume_partial_nodes(1)?;
    ...
    fn make_known_sum(
        ...
        if let Some(values) = all_static(&fields) {
            return self.make_known(SpecializationValue::Sum {
                ty: sum,
                constructor,
                fields: values,
            });
    ...
    fn entry_partial(
        ...
        match slot {
            ...
            _ => Err(PolyvariantR1S3Error::InvalidEntrySlot {
    ...
    fn admit_reachable_subset(
        &mut self,
        entry: FunctionId,
    ) -> Result<BTreeMap<FunctionId, Vec<FunctionId>>, PolyvariantR1S3Error> {
        ...
        while let Some(function_id) = pending.pop() {
            ...
            let function = self
                ...
                .get(&function_id)
                .cloned()
                .ok_or(PolyvariantR1S3Error::MissingFunction(function_id))?;
            ...
            for callee in callees.iter().rev() {
                if !self.functions.contains_key(callee) {
                    return Err(PolyvariantR1S3Error::MissingFunction(*callee));
                }
                pending.push(*callee);
            }
    ...
    fn admit_function(&mut self, function: &Function) -> Result<(), PolyvariantR1S3Error> {
        self.consume_work(1)?;
        if !matches!(function.region_parameters.as_slice(), [] | [RegionId(0)]) {
            return Err(PolyvariantR1S3Error::UnsupportedRegionParameters {
                function: function.id,
            });
        }
        if !is_admitted_effect_row(&function.effects) {
            return Err(PolyvariantR1S3Error::UnsupportedEffects {
                function: function.id,
            });
        }
    ...
    fn admit_type(
        ...
        match ty {
            ...
            _ => Err(PolyvariantR1S3Error::UnsupportedType {
                function,
                context,
                ty: ty.clone(),
            }),
    ...
    fn admit_term(
        ...
        match term {
            ...
            Term::Region { .. } => Err(PolyvariantR1S3Error::UnsupportedNode {
                function,
                node: "Region",
            }),
            Term::Handle { .. } => Err(PolyvariantR1S3Error::UnsupportedNode {
                function,
                node: "Handle",
            }),
        }
        ...
        match value {
            ...
            RValue::Primitive {
                ...
                match operation {
                    ...
                    _ => Err(PolyvariantR1S3Error::UnsupportedPrimitive {
                        function,
            ...
            | RValue::PackClosure { .. }
            | RValue::CallClosure { .. }
            | RValue::Perform { .. } => Err(PolyvariantR1S3Error::UnsupportedNode {
                function,
    ...
    fn admit_operand(&mut self, _operand: &Operand) -> Result<(), PolyvariantR1S3Error> {
        self.consume_work(1)
    }

    fn admit_operands(&mut self, operands: &[Operand]) -> Result<(), PolyvariantR1S3Error> {
    ...
    fn patterns_and_operands(
        &mut self,
        values: &[Partial],
    ) -> Result<(Vec<PolyvariantR1S3Pattern>, Vec<Operand>), PolyvariantR1S3Error> {
    ...
    fn pattern_for(
    ...
    ) -> Result<PolyvariantR1S3Pattern, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        Ok(match value.as_ref() {
            ...
            PartialValue::Tuple(fields) => {
                ...
                PolyvariantR1S3Pattern::Tuple(patterns)
            }
            PartialValue::KnownSum {
                ...
                PolyvariantR1S3Pattern::KnownSum {
                    sum: sum.clone(),
                    constructor: *constructor,
                    fields: patterns,
                }
            ...
            PartialValue::UnknownSum { sum, atom } => {
                let alias = self.alias_for(atom, aliases, operands)?;
                PolyvariantR1S3Pattern::UnknownSum {
                    sum: sum.clone(),
                    alias,
                }
            }
    ...
    fn alias_for(
        ...
        aliases: &mut BTreeMap<AtomId, (u32, Type, Operand)>,
        operands: &mut Vec<Operand>,
    ) -> Result<u32, PolyvariantR1S3Error> {
        if let Some((alias, ty, _)) = aliases.get(&atom.id) {
            if *ty != atom.ty {
                return Err(PolyvariantR1S3Error::InternalInvariant {
                    message: "one R1-S3 atom acquired two types".to_owned(),
                });
            }
            return Ok(*alias);
        }
        let alias = u32::try_from(aliases.len()).map_err(|_| {
            PolyvariantR1S3Error::DynamicParameterBudgetExceeded {
    ...
    fn reserve(&mut self, key: VersionKey) -> Result<FunctionId, PolyvariantR1S3Error> {
        ...
        if self.usage.variants == self.budget.max_variants {
            return Err(PolyvariantR1S3Error::VariantBudgetExceeded {
        ...
        let next_dynamic = self
            ...
            .ok_or(PolyvariantR1S3Error::DynamicParameterBudgetExceeded {
                limit: self.budget.max_dynamic_parameters,
            })?;
        if next_dynamic > self.budget.max_dynamic_parameters {
            return Err(PolyvariantR1S3Error::DynamicParameterBudgetExceeded {
                limit: self.budget.max_dynamic_parameters,
            });
        }
        let temporary_id = u32::try_from(self.usage.variants)
            .map(FunctionId)
            .map_err(|_| PolyvariantR1S3Error::FunctionIdExhausted)?;
    ...
    fn build_version(&mut self, key: VersionKey) -> Result<(), PolyvariantR1S3Error> {
        self.consume_work(1)?;
        let reserved = self
            .reserved
            .get(&key)
            .cloned()
            .ok_or(PolyvariantR1S3Error::UnresolvedVariant)?;
        if reserved.built {
            return Ok(());
        }
        let source = self
            ...
            .cloned()
            .ok_or(PolyvariantR1S3Error::MissingFunction(key.source_function))?;
        ...
        for (expected_alias, (alias, ty)) in aliases.into_iter().enumerate() {
            if alias as usize != expected_alias {
                return Err(PolyvariantR1S3Error::InternalInvariant {
                    message: "R1-S3 pattern aliases are not contiguous".to_owned(),
                });
            }
            ...
            .zip(source.parameters.iter().map(|parameter| &parameter.ty))
        {
            ...
            if partial_type(&value)? != *expected_type {
                return Err(PolyvariantR1S3Error::InternalInvariant {
                    message: format!(
                        "R1-S3 pattern type does not match source parameter {}",
                        parameter.local.0
                    ),
                });
            }
    ...
    fn partial_from_pattern(
        &mut self,
        pattern: &PolyvariantR1S3Pattern,
        aliases: &BTreeMap<u32, Atom>,
    ) -> Result<Partial, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        match pattern {
            PolyvariantR1S3Pattern::KnownUnit => self.make_known(SpecializationValue::Unit),
            PolyvariantR1S3Pattern::KnownBool(value) => {
                self.make_known(SpecializationValue::Bool(*value))
            }
            PolyvariantR1S3Pattern::KnownI64(value) => {
                self.make_known(SpecializationValue::I64(*value))
            }
            PolyvariantR1S3Pattern::KnownF64(bits) => {
                self.make_known(SpecializationValue::F64(f64::from_bits(*bits)))
            }
            PolyvariantR1S3Pattern::KnownTuple(fields) => {
            ...
            PolyvariantR1S3Pattern::Hole { ty, alias } => {
                let atom = aliases.get(alias).cloned().ok_or_else(|| {
                    PolyvariantR1S3Error::InternalInvariant {
                        message: format!("R1-S3 pattern references missing alias {alias}"),
                    }
                })?;
                if atom.ty != *ty || matches!(ty, Type::Sum(_)) {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 Hole pattern has an invalid alias type".to_owned(),
                    });
                }
                self.make_dynamic(ty.clone(), atom.id, atom.operand)
            }
            PolyvariantR1S3Pattern::Tuple(fields) => {
                let mut partials = Vec::with_capacity(fields.len());
                for field in fields {
            ...
            PolyvariantR1S3Pattern::UnknownSum { sum, alias } => {
                let atom = aliases.get(alias).cloned().ok_or_else(|| {
                    PolyvariantR1S3Error::InternalInvariant {
                        message: format!("R1-S3 pattern references missing alias {alias}"),
                    }
                ...
                if atom.ty != Type::Sum(sum.clone()) {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 UnknownSum alias type changed".to_owned(),
                    });
                }
    ...
    ) -> Result<Term, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        match term {
            Term::Let {
                binder,
            ...
            Term::If {
                ...
                match condition.as_ref() {
                    PartialValue::Known(value) => match value.as_ref() {
                        ...
                        _ => Err(PolyvariantR1S3Error::ExpectedBool { function }),
                    },
    ...
    fn specialize_case(
        ...
        if let Some((sum, constructor, fields)) = self.known_sum_parts(scrutinee)? {
            let arm = arms
                ...
                .ok_or(PolyvariantR1S3Error::ExpectedSum { function })?;
            let constructor_type = sum
                .constructors
                .get(constructor as usize)
                .ok_or(PolyvariantR1S3Error::ExpectedSum { function })?;
            if arm.bindings.len() != fields.len() || constructor_type.fields.len() != fields.len() {
                return Err(PolyvariantR1S3Error::ExpectedSum { function });
            }
        ...
        for arm in arms {
            ...
            for (index, (binding, ty)) in arm.bindings.iter().zip(&constructor.fields).enumerate() {
                let field =
                    u32::try_from(index).map_err(|_| PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 Case field index exceeds U32".to_owned(),
                    })?;
    ...
    fn known_sum_parts(
        &mut self,
        value: &Partial,
    ) -> Result<Option<(SumType, u32, Vec<Partial>)>, PolyvariantR1S3Error> {
        match value.as_ref() {
            PartialValue::Known(value) => {
    ...
    fn specialize_rvalue(
        ...
        value: &RValue,
        environment: &BTreeMap<LocalId, Partial>,
    ) -> Result<SpecializedRValue, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        match value {
                ...
                match tuple.as_ref() {
                    PartialValue::Known(value) => {
                        let SpecializationValue::Tuple(fields) = value.as_ref() else {
                            return Err(PolyvariantR1S3Error::ExpectedTuple { function });
                        };
                        let field = fields
                            .get(*index as usize)
                            .ok_or(PolyvariantR1S3Error::ExpectedTuple { function })?;
                    ...
                    _ => Err(PolyvariantR1S3Error::ExpectedTuple { function }),
                }
            }
            RValue::Construct {
                sum,
        ...
        match operand {
            ...
            Operand::Local(local) => {
                environment
                    .get(local)
                    .cloned()
                    .ok_or(PolyvariantR1S3Error::MissingLocal {
                        function,
                        local: *local,
                    })
            }
    ...
    fn resolve_operands(
        ...
        operands: &[Operand],
        environment: &BTreeMap<LocalId, Partial>,
    ) -> Result<Vec<Partial>, PolyvariantR1S3Error> {
        operands
            .iter()
    ...
    fn reserve_call(
        ...
        function: FunctionId,
        arguments: &[Partial],
    ) -> Result<(FunctionId, Vec<Operand>), PolyvariantR1S3Error> {
        let callee = self
            .functions
            .get(&function)
            .ok_or(PolyvariantR1S3Error::MissingFunction(function))?;
    ...
    fn try_unfold_helper(
        ...
        function: FunctionId,
        arguments: &[Partial],
    ) -> Result<Option<Partial>, PolyvariantR1S3Error> {
        ...
        loop {
            ...
            let cursor = frames
                .last()
                .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                    message: "R1-S3 helper machine lost its entry frame".to_owned(),
                })?
                .cursor
                .clone();
            match cursor {
                Term::Let {
                    ...
                    let current_function = frames
                        .last()
                        .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 helper machine lost its current frame".to_owned(),
                        })?
                        .function;
                    let environment = frames
                        .last()
                        .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 helper machine lost its environment".to_owned(),
                        })?
                        .environment
                        .clone();
                    if let RValue::Call {
                    ...
                    } = value
                    {
                        ...
                        frames
                            .last_mut()
                            .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper machine lost its caller frame".to_owned(),
                            })?
                            .cursor = *next;
                        frames.push(frame);
                        continue;
                    ...
                    let frame = frames.last_mut().ok_or_else(|| {
                        PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 helper machine lost its binding frame".to_owned(),
                        }
                    })?;
                ...
                Term::Return(operand) => {
                    let frame =
                        frames
                            .last()
                            .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper machine lost its return frame".to_owned(),
                            })?;
                    let result =
                    ...
                    let completed =
                        frames
                            .pop()
                            .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper machine could not pop its return frame"
                                    .to_owned(),
                            })?;
                    ...
                    if let Some(binder) = completed.return_to {
                        let caller = frames.last_mut().ok_or_else(|| {
                            PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper return has no caller frame".to_owned(),
                            }
                        })?;
                        ...
                        if !frames.is_empty() {
                            return Err(PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper entry returned with live frames".to_owned(),
                            });
                        }
                ...
                Term::If {
                    ...
                    let frame =
                        frames
                            .last()
                            .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper machine lost its If frame".to_owned(),
                            })?;
                    let condition =
                    ...
                    frames
                        .last_mut()
                        .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 helper machine lost its selected If frame".to_owned(),
                        })?
                        .cursor = selected;
                }
                Term::Case { scrutinee, arms } => {
                    let frame =
                        frames
                            .last()
                            .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                                message: "R1-S3 helper machine lost its Case frame".to_owned(),
                            })?;
                    let scrutinee =
                    ...
                    let frame = frames.last_mut().ok_or_else(|| {
                        PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 helper machine lost its selected Case frame".to_owned(),
                        }
                    })?;
                    for (binding, field) in arm.bindings.iter().zip(fields) {
    ...
    fn make_helper_frame(
        ...
        depth: usize,
    ) -> Result<Option<HelperFrame>, PolyvariantR1S3Error> {
        if self.recursive_functions.contains(&function)
            || depth == R1_S3_MAX_HELPER_DEPTH
    ...
    fn is_bounded_helper_metered(&mut self, term: &Term) -> Result<bool, PolyvariantR1S3Error> {
    ...
    ) -> Result<Operand, PolyvariantR1S3Error> {
        self.consume_work(1)?;
        match value.as_ref() {
            ...
            PartialValue::Hole(atom) => {
                if atom.ty != *expected {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 Hole materialization type mismatch".to_owned(),
                    });
                }
            ...
            PartialValue::Tuple(fields) => {
                let Type::Tuple(types) = expected else {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 Tuple materialized at non-Tuple type".to_owned(),
                    });
                };
                if fields.len() != types.len() {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 Tuple materialization arity mismatch".to_owned(),
                    });
                }
            ...
            PartialValue::KnownSum {
                ...
                if *expected != Type::Sum(sum.clone()) {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 KnownSum materialization type mismatch".to_owned(),
                    });
                }
                let constructor_type =
                    sum.constructors.get(*constructor as usize).ok_or_else(|| {
                        PolyvariantR1S3Error::InternalInvariant {
                            message: "R1-S3 KnownSum constructor is absent".to_owned(),
                        }
                    })?;
                if fields.len() != constructor_type.fields.len() {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 KnownSum materialization arity mismatch".to_owned(),
                    });
                }
                let mut operands = Vec::with_capacity(fields.len());
            ...
            PartialValue::UnknownSum { sum, atom } => {
                if *expected != Type::Sum(sum.clone()) || atom.ty != *expected {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 UnknownSum materialization type mismatch".to_owned(),
                    });
                }
                Ok(atom.operand.clone())
            }
    ...
    fn materialize_known(
        ...
        if let Some(literal) = literal_from_value(value) {
            ...
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 known scalar materialization type mismatch".to_owned(),
            });
        }
        match (value, expected) {
            ...
            (
                ...
                let constructor_type = expected_sum
                    ...
                    .get(*constructor as usize)
                    .ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 known Sum constructor is absent".to_owned(),
                    })?;
                if fields.len() != constructor_type.fields.len() {
                    return Err(PolyvariantR1S3Error::InternalInvariant {
                        message: "R1-S3 known Sum materialization arity mismatch".to_owned(),
                    });
                }
                let mut operands = Vec::with_capacity(fields.len());
                for (field, ty) in fields.iter().zip(&constructor_type.fields) {
            ...
            _ => Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 cannot materialize the known value at its result type".to_owned(),
            }),
        }
    }
    ...
    fn lower_program(
        ...
        profile: CoreProfile,
    ) -> Result<(Program, Vec<PolyvariantR1S3Variant>, PolyvariantR1S3Usage), PolyvariantR1S3Error>
    {
        ...
        for pair in keyed_versions.windows(2) {
            if pair[0].0 == pair[1].0 && pair[0].1.key != pair[1].1.key {
                return Err(PolyvariantR1S3Error::InternalInvariant {
                    message: "two distinct R1-S3 version keys have identical bytes".to_owned(),
                });
            }
        }
        ...
        for mut version in built {
            ...
            variants.push(PolyvariantR1S3Variant {
...
fn validate_budget(budget: PolyvariantR1S3Budget) -> Result<(), PolyvariantR1S3Error> {
    for (field, limit, hard_cap) in [
        ...
        if limit == 0 {
            return Err(PolyvariantR1S3Error::ZeroBudget { field });
        }
        if limit > hard_cap {
            return Err(PolyvariantR1S3Error::BudgetHardCapExceeded {
                field,
                limit,
                hard_cap,
            });
...
fn specialization_value_nodes(value: &SpecializationValue) -> Result<u64, PolyvariantR1S3Error> {
    let children = match value {
        ...
        SpecializationValue::ArrayF64(_) => {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 counted an excluded static array".to_owned(),
            });
        }
    };
    let mut nodes = 1_u64;
    for child in children {
        nodes = nodes
            ...
            .ok_or(PolyvariantR1S3Error::PartialValueBudgetExceeded {
                limit: R1_S3_MAX_PARTIAL_VALUE_NODES_HARD_CAP,
            })?;
    }
    Ok(nodes)
}

fn specialization_value_type(value: &SpecializationValue) -> Result<Type, PolyvariantR1S3Error> {
    Ok(match value {
        SpecializationValue::Unit => Type::Unit,
        SpecializationValue::Bool(_) => Type::Bool,
        SpecializationValue::I64(_) => Type::I64,
        ...
        SpecializationValue::ArrayF64(_) => {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 inferred the type of an excluded static array".to_owned(),
            });
        }
    })
}
...
fn partial_type(value: &Partial) -> Result<Type, PolyvariantR1S3Error> {
    match value.as_ref() {
...
fn known_pattern(
    ...
    Ok(match value {
        SpecializationValue::Unit => PolyvariantR1S3Pattern::KnownUnit,
        SpecializationValue::Bool(value) => PolyvariantR1S3Pattern::KnownBool(*value),
        SpecializationValue::I64(value) => PolyvariantR1S3Pattern::KnownI64(*value),
        SpecializationValue::F64(value) => {
            PolyvariantR1S3Pattern::KnownF64(canonical_f64_bits(*value))
        }
        SpecializationValue::Tuple(fields) => PolyvariantR1S3Pattern::KnownTuple(
            fields
                .iter()
                .map(known_pattern)
                .collect::<Result<Vec<_>, _>>()?,
        ...
        SpecializationValue::ArrayF64(_) => {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 encoded an excluded static array pattern".to_owned(),
            });
        }
    })
}

fn scalar_operand(value: &Partial) -> Result<Operand, PolyvariantR1S3Error> {
    match value.as_ref() {
        PartialValue::Known(value) => {
            literal_from_value(value).ok_or_else(|| PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 primitive received a known aggregate".to_owned(),
            })
        }
        PartialValue::Hole(atom)
        ...
        _ => Err(PolyvariantR1S3Error::InternalInvariant {
            message: "R1-S3 primitive received a structural value".to_owned(),
        }),
    }
}

fn array_operand(value: &Partial) -> Result<Operand, PolyvariantR1S3Error> {
    match value.as_ref() {
        PartialValue::Hole(atom)
            if matches!(
                atom.ty,
        ...
        {
            Err(PolyvariantR1S3Error::InternalInvariant {
                message: "R1-S3 cannot materialize a static array literal".to_owned(),
            })
        }
        _ => Err(PolyvariantR1S3Error::InternalInvariant {
            message: "R1-S3 array primitive received a non-array value".to_owned(),
        }),
    }
}
...
) -> Result<SpecializationValue, PolyvariantR1S3Error> {
    match primitive {
        ...
        Primitive::ArrayLenF64 | Primitive::ArrayGetF64 => {
            Err(PolyvariantR1S3Error::InternalInvariant {
                message: "array primitive passed R1-S3 admission".to_owned(),
            })
        }
    }
}
...
fn apply_i64(
...
) -> Result<i64, PolyvariantR1S3Error> {
    Ok(match (mode, operation) {
        ...
        (NumericMode::Checked, _) => {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "checked arithmetic passed R1-S3 admission".to_owned(),
            });
        }
    })
}

fn i64_pair(values: &[SpecializationValue]) -> Result<(i64, i64), PolyvariantR1S3Error> {
    let [SpecializationValue::I64(left), SpecializationValue::I64(right)] = values else {
        return Err(PolyvariantR1S3Error::InternalInvariant {
            message: "verified R1-S3 I64 primitive argument mismatch".to_owned(),
        });
    };
    Ok((*left, *right))
}

fn f64_pair(values: &[SpecializationValue]) -> Result<(f64, f64), PolyvariantR1S3Error> {
    let [SpecializationValue::F64(left), SpecializationValue::F64(right)] = values else {
        return Err(PolyvariantR1S3Error::InternalInvariant {
            message: "verified R1-S3 F64 primitive argument mismatch".to_owned(),
        });
    };
    Ok((*left, *right))
}

fn pattern_nodes(patterns: &[PolyvariantR1S3Pattern]) -> Result<u64, PolyvariantR1S3Error> {
    let mut nodes = 0_u64;
    for pattern in patterns {
        nodes = nodes.checked_add(pattern_node_count(pattern)?).ok_or(
            PolyvariantR1S3Error::WorkBudgetExceeded {
                limit: R1_S3_MAX_WORK_UNITS_HARD_CAP,
            },
        )?;
    }
    Ok(nodes)
...
fn pattern_node_count(pattern: &PolyvariantR1S3Pattern) -> Result<u64, PolyvariantR1S3Error> {
    let mut nodes = 1_u64;
    match pattern {
        PolyvariantR1S3Pattern::KnownTuple(fields) | PolyvariantR1S3Pattern::Tuple(fields) => {
            for field in fields {
        ...
        PolyvariantR1S3Pattern::Hole { ty, .. } => {
        ...
        PolyvariantR1S3Pattern::KnownSum { sum, fields, .. } => {
        ...
        PolyvariantR1S3Pattern::UnknownSum { sum, .. } => {
            nodes = nodes.checked_add(sum_type_node_count(sum)?).ok_or(
                PolyvariantR1S3Error::WorkBudgetExceeded {
                    limit: R1_S3_MAX_WORK_UNITS_HARD_CAP,
                },
        ...
        PolyvariantR1S3Pattern::KnownUnit
        | PolyvariantR1S3Pattern::KnownBool(_)
        | PolyvariantR1S3Pattern::KnownI64(_)
        | PolyvariantR1S3Pattern::KnownF64(_) => {}
    }
    Ok(nodes)
}

fn type_node_count(ty: &Type) -> Result<u64, PolyvariantR1S3Error> {
    let mut nodes = 1_u64;
    match ty {
        Type::Tuple(fields) => {
            for field in fields {
...
fn sum_type_node_count(sum: &SumType) -> Result<u64, PolyvariantR1S3Error> {
    let mut nodes = 1_u64;
    for constructor in &sum.constructors {
        nodes = nodes
            .checked_add(1)
...
fn collect_pattern_aliases(
    patterns: &[PolyvariantR1S3Pattern],
) -> Result<BTreeMap<u32, Type>, PolyvariantR1S3Error> {
...
fn collect_pattern_alias(
    ...
    match pattern {
        ...
        PolyvariantR1S3Pattern::KnownTuple(fields)
        | PolyvariantR1S3Pattern::Tuple(fields)
        | PolyvariantR1S3Pattern::KnownSum { fields, .. } => {
            for field in fields {
        ...
        | PolyvariantR1S3Pattern::KnownBool(_)
        | PolyvariantR1S3Pattern::KnownI64(_)
        | PolyvariantR1S3Pattern::KnownF64(_) => Ok(()),
    }
}
...
fn insert_alias(
    ...
    if let Some(existing) = aliases.get(&alias) {
        if *existing != ty {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: format!("R1-S3 alias {alias} has inconsistent types"),
            });
        }
    } else {
...
fn version_key_bytes(key: &VersionKey) -> Result<Vec<u8>, PolyvariantR1S3Error> {
...
fn encode_pattern(
    ...
    match pattern {
        PolyvariantR1S3Pattern::KnownUnit => bytes.push(0),
        PolyvariantR1S3Pattern::KnownBool(value) => {
            bytes.push(1);
...
fn encode_type(bytes: &mut Vec<u8>, ty: &Type) -> Result<(), PolyvariantR1S3Error> {
    match ty {
        ...
        _ => {
            return Err(PolyvariantR1S3Error::InternalInvariant {
                message: "excluded type entered the R1-S3 key encoder".to_owned(),
            });
        }
    }
    Ok(())
...
fn encode_sum_type(bytes: &mut Vec<u8>, sum: &SumType) -> Result<(), PolyvariantR1S3Error> {
...
fn put_len(
    ...
    let len = u32::try_from(len).map_err(|_| PolyvariantR1S3Error::InternalInvariant {
        message: format!("R1-S3 {context} exceeds U32"),
    })?;
...
fn term_node_count(term: &Term) -> Result<u64, PolyvariantR1S3Error> {
