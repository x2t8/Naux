...
fn main() -> ExitCode {
    ...
    let Some(case_ordinal) = arguments.next() else {
        eprintln!("naux-policy15-candidate-worker requires exactly one case ordinal");
        return ExitCode::from(EXIT_USAGE);
    };
    if arguments.next().is_some() {
        eprintln!("naux-policy15-candidate-worker accepts exactly one case ordinal");
        return ExitCode::from(EXIT_USAGE);
    }
    let Some(canonical_case_ordinal) = case_ordinal.to_str() else {
        eprintln!("candidate worker case ordinal is not canonical UTF-8");
        return ExitCode::from(EXIT_USAGE);
    };
    let Ok(case_ordinal) = canonical_case_ordinal.parse::<u32>() else {
        eprintln!("candidate worker case ordinal is not a canonical u32");
        return ExitCode::from(EXIT_USAGE);
    };
    if case_ordinal.to_string() != canonical_case_ordinal {
        eprintln!("candidate worker case ordinal is not canonical decimal");
        return ExitCode::from(EXIT_USAGE);
    }
    ...
    let frame = match naux::core::emit_x64_gate_b_policy15_candidate_worker_frame(case_ordinal) {
        Ok(frame) => frame,
        Err(error) => {
            eprintln!("naux-policy15-candidate-worker failed: {error}");
...
fn run_debug_probe(case_ordinal: u32) -> Option<ExitCode> {
    let mode = std::env::var_os("NAUX_POLICY15_CANDIDATE_WORKER_DEBUG_PROBE")?;
    match mode.to_str()? {
        ...
        "descendant-pipe" => {
            ...
            Command::new(executable)
                ...
                .env(
                    "NAUX_POLICY15_CANDIDATE_WORKER_DEBUG_PROBE",
                    "descendant-holder",
                )
                .stdin(Stdio::null())
        ...
        "trailing" => {
            let mut frame =
                naux::core::emit_x64_gate_b_policy15_candidate_worker_frame(case_ordinal).ok()?;
            frame.push(0);
        ...
        "wrong-case" => {
            ...
            let frame =
                naux::core::emit_x64_gate_b_policy15_candidate_worker_frame(wrong_case).ok()?;
            let _ = io::stdout().lock().write_all(&frame);
            Some(ExitCode::SUCCESS)
        }
        _ => None,
