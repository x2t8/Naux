...
use naux::core::{
    emit_x64_tail_candidate_capsule, emit_x64_tail_physical_allocation, emit_x64_tail_state_plan,
    emit_x64_tail_template_realization, lower_core_ssa_r1_s5, lower_machine_ir_r1_s6,
    lower_x64_target_r1_s7a, verify_x64_tail_candidate_capsule, CoreArtifact, CoreProfile,
...
#[test]
fn adr0060_public_boundary_owns_bytes_without_executable_authority() {
    ...
    assert_eq!(X64_TAIL_CANDIDATE_CAPSULE_SCHEMA_VERSION, (1, 0, 0));
    assert_eq!(X64_TAIL_CANDIDATE_CAPSULE_POLICY_VERSION, (1, 1, 0));
    assert_eq!(X64_TARGET_ENCODER_POLICY_VERSION, (1, 4, 0));
    assert!(!verified.capsule().code().is_empty());
    assert_eq!(
        verified.capsule().source_target_semantic_hash(),
        target.semantic_hash
    );
    assert_eq!(
        verified.capsule().source_template_realization_hash(),
