...
fn action_variants_emit_events() {
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e, RuntimeEvent::Ui { kind, .. } if kind == "card")));
    assert!(events
        .iter()
        .any(|e| matches!(e, RuntimeEvent::Text(t) if t == "hello")));
    assert!(events
        .iter()
        .any(|e| matches!(e, RuntimeEvent::Button(b) if b == "ok")));
    assert!(events
        .iter()
        .any(|e| matches!(e, RuntimeEvent::Log(l) if l == "log")));
}
