...
use naux::core::{
    ...
    emit_x64_tail_physical_allocation, emit_x64_tail_site_binding_proof, emit_x64_tail_state_plan,
    emit_x64_tail_template_realization, lower_core_ssa_r1_s5, lower_machine_ir_r1_s6,
    lower_x64_target_r1_s7a, verify_x64_tail_body_frontier_realization, CoreArtifact, CoreProfile,
    EffectRow, Function, FunctionId, Program, SchemaVersion, Term, Type,
    X64_TAIL_BODY_FRONTIER_POLICY_VERSION, X64_TAIL_BODY_FRONTIER_SCHEMA_VERSION,
...
#[test]
fn adr0062_public_boundary_replays_symbolic_state_without_new_bytes() {
    ...
    let verified = verify_x64_tail_body_frontier_realization(
        &realization,
        &binding,
        &capsule,
        &tail_templates,
    ...
    assert_eq!(X64_TAIL_BODY_FRONTIER_SCHEMA_VERSION, (1, 1, 0));
    assert_eq!(X64_TAIL_BODY_FRONTIER_POLICY_VERSION, (1, 2, 0));
    assert_eq!(
        verified.realization().source_target_semantic_hash(),
        target.semantic_hash
    );
    assert_eq!(
        verified.realization().source_site_binding_hash(),
        binding.proof_hash()
    );
