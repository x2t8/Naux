...
NAUX Learn 0.1.3 is an unsigned experimental pre-release for Linux x86-64 GNU.
...
The bootstrap is pinned to version `0.1.3`, verifies the archive SHA-256, asks
...
Expected version output is `naux 0.1.3`.

For an inspect-before-execute workflow, download `nauxup.sh`, the `.tar.gz`,
and `SHA256SUMS` from the
[GitHub pre-release](https://github.com/x2t8/Naux/releases/tag/v0.1.3-learn).
Review the bootstrap and `PROVENANCE.tsv`, verify the archive with
`sha256sum -c SHA256SUMS`, then run `sh nauxup.sh`.
