...
def emit_rocq(kernels: list[Kernel], report_root: str) -> str:
    rows = [
        ...
        "  Exact terminator operands, per-block selection parity, bounded",
        "  dynamic CFG path construction, and all terminating control",
        "  derivations are retained. Host failures, counter exhaustion, total",
        "  termination, and native semantics remain explicit non-claims.",
        "*)",
        "",
    ...
    for kernel in kernels:
        ...
        rows.extend(
            [
                ...
                "Qed.",
                "",
                f"Theorem {prefix}_all_terminating_control_executions_are_abi_correct :",
                "  forall initial replacement final_initialized baseline_out value,",
                "    ownership_control_baseline_terminates",
                f"      {_coq_nat(kernel.home_slot)} {prefix}_control_graph",
                "      0%nat false initial final_initialized baseline_out value ->",
                "    exists candidate_out,",
                "      ownership_control_candidate_terminates",
                f"        {_coq_nat(kernel.home_slot)} 0%nat",
                f"        {prefix}_control_graph 0%nat false",
                "        (ownership_hide_reserved_register",
                "          0%nat replacement initial)",
                ...
                "          final_initialized candidate_out).",
                "Proof.",
                "  intros initial replacement final_initialized baseline_out value",
                "    Hbaseline.",
                "  eapply ownership_control_terminating_abi_correct",
                f"    with (graph := {prefix}_control_graph).",
                "  - reflexivity.",
                "  - exact Hbaseline.",
                "Qed.",
