...
cc -O3 -march=native -o "$ROOT_DIR/benchmarks/c/bench_sum_dense" "$ROOT_DIR/benchmarks/c/bench_sum_dense.c"
taskset -c 0 "$ROOT_DIR/benchmarks/c/bench_sum_dense" 100000 "$ITERS" "$WARMUP_MS" "$REPS"
cc -O3 -march=native -o "$ROOT_DIR/benchmarks/c/bench_list_update" "$ROOT_DIR/benchmarks/c/bench_list_update.c"
taskset -c 0 "$ROOT_DIR/benchmarks/c/bench_list_update" 100000 "$ITERS" "$WARMUP_MS" "$REPS"
cc -O3 -march=native -o "$ROOT_DIR/benchmarks/c/bench_dot_product" "$ROOT_DIR/benchmarks/c/bench_dot_product.c"
taskset -c 0 "$ROOT_DIR/benchmarks/c/bench_dot_product" 100000 "$ITERS" "$WARMUP_MS" "$REPS"
